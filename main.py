...
def dicom_scan(InputPath, OutputPath=None, Compression=None):
    ...
    filesize = 0
    counter = 0
    directories_df = pd.DataFrame({'PatientName': [], 'StudyID' : [], 'Path' : []})
    ...
    for dcm_file in pathlib.Path(InputPath).rglob('*'):
        ...
        try:
            ...
            directorypath = str(os.path.abspath(dcm_file))
            new_row_directories = pd.DataFrame({'PatientName': [patientsname],'StudyID' : [dcm.StudyID] , 'Path' : [directorypath]})
    ...
    for path in directories_df['Path']:
        shutil.move(path, OutputPath)
